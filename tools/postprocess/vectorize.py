...
from tools.postprocess.cluster import LaneNetPostProcessor
from tools.postprocess.connect import sort_points_by_dist, connect_by_direction
...
def vectorize(segmentation, embedding, direction, angle_class=36):
    ...
    for i in range(1, oh_pred.shape[0]):
        ...
        single_embedding = embedding.permute(1, 2, 0)#400 200 16
...
if __name__ == '__main__':
    a = torch.rand((4,400,200))
    b = torch.rand((16,400,200))
    c = torch.rand((37,400,200))
    coords, confidences, line_types = vectorize(a, b, c)
