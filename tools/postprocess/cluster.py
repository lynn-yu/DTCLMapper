...
class _LaneNetCluster(object):
    ...
    def _embedding_feats_dbscan_cluster(self, embedding_image_feats):
        ...
        # db = MeanShift()
        try:
            features = StandardScaler().fit_transform(embedding_image_feats)#标准化特征
            db.fit(features)#聚类
    ...
    def _get_lane_embedding_feats(binary_seg_ret, instance_seg_ret):
        ...
        :return:
        """
        idx = np.where(binary_seg_ret == 255)#取出目标的位置
        lane_embedding_feats = instance_seg_ret[idx]#取出对应的实例特征
...
class LaneNetPostProcessor(object):
    ...
    def postprocess(self, binary_seg_result, instance_seg_result=None, min_area_threshold=100):
        ...
        :param data_source:
        :return:
        通过语义分割找到每个类别的位置，然后提取这些位置的实例特征，然后通过聚类算法，分割这些实例特征
        """
        ...
        connect_components_analysis_ret = _connect_components_analysis(image=morphological_ret)#检测连通域

        labels = connect_components_analysis_ret[1]#400 200
        stats = connect_components_analysis_ret[2]#stats：包含5个参数，分别是，x，y，w，h，s。分别对应每个连通区域的外接矩形的左上角坐标x和y，以及其宽和高，s是指的连通域中的像素个数（不是指整个外接矩形的面积）
        for index, stat in enumerate(stats):
            if stat[4] <= min_area_threshold:#连通域像素个数小勺的
                idx = np.where(labels == index)
                morphological_ret[idx] = 0#删除
