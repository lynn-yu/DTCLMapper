...
class MapTRAssigner(BaseAssigner):
    ...
    def assign(self,
        ...
        cls_cost = self.cls_cost(cls_pred, gt_labels)#50 8
        # regression L1 cost
        ...
        pts_cost_ordered = self.pts_cost(pts_pred_interpolated, normalized_gt_pts)#50 152= 50 label*19
        pts_cost_ordered = pts_cost_ordered.view(num_bboxes, num_gts, num_orders)#50 label 19
        pts_cost, order_index = torch.min(pts_cost_ordered, 2)#50 label
        
        bboxes = denormalize_2d_bbox(bbox_pred, self.pc_range)
        iou_cost = self.iou_cost(bboxes, gt_bboxes)#50 8
        ...
        return AssignResult(
            num_gts, assigned_gt_inds, None, labels=assigned_labels), order_index,cost
