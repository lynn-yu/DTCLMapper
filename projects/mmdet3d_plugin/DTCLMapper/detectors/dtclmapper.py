...
class DTCLMapper(MVXTwoStageDetector):
    """MapTR.
    Args:
    ...
    def __init__(self,
        ...
        # self.merge=nn.Sequential(nn.Conv2d(256*2, 256, 3, padding=1, bias=False),
        #     nn.BatchNorm2d(256),
        #     nn.ReLU(True))
    ...
    def forward_pts_train(self,
                          ...
                          semantic_gt=None,
                          img_feats_pre=None,
                          prev_gt_bboxes_3d=None, prev_gt_labels_3d=None,gt_depth=None,
                          ):
        """Forward function'
        Args:
        ...
        """
        pre = prev_bev['bev_embed']#
        losses = dict()


        outs = self.pts_bbox_head(
            pts_feats, lidar_feat, img_metas, prev_bev=pre,pre_feat=img_feats_pre,mask=semantic_gt,train=True)
        # if gt_depth is not None and outs['depth'] is not None:
        #     loss_depth = self.pts_bbox_head.transformer.encoder.get_depth_loss(gt_depth, outs['depth'])
        #     if digit_version(TORCH_VERSION) >= digit_version('1.8'):
        #         loss_depth = torch.nan_to_num(loss_depth)
        #     losses.update(loss_depth=loss_depth)

        self.pts_bbox_head.pre_memory_update(prev_gt_bboxes_3d, prev_gt_labels_3d, prev_bev,pre=True)
        loss_inputs = [semantic_mask, instance_mask, gt_bboxes_3d, gt_labels_3d, outs, pre]
    ...
    def obtain_history_bev(self, imgs, img_metas):
        ...
        with torch.no_grad():
            if len_queue>1:
                ...
                img_feats= self.extract_feat(img=img,img_metas=prev_img_metas)
                prev_bev =self.pts_bbox_head(img_feats, None,prev_img_metas,prev_bev=None, only_bev=True)#
                prev_bev = rearrange(prev_bev, 'b hw c -> hw b c')
            else:
                prev_bev=None
            img = imgs[:, len_queue-1]
            prev_img_metas = [each[len_queue-1] for each in img_metas]
            img_feats = self.extract_feat(img=img, img_metas=prev_img_metas)
            prev_bev = self.pts_bbox_head(img_feats, None, prev_img_metas, prev_bev=prev_bev)  # , only_bev=True
    ...
    #@force_fp32(apply_to=('img','points'))#取消注释，时序融合运行有bug
    def forward_train(self,
        ...
        img = img[:, -1, ...]
        b = img.shape[0]
        prev_img_metas = img_metas#[each[0] for each in img_metas]
        def show(img):
            plt.figure()
            plt.imshow(img)
            plt.show()
        # show(semantic_mask[0]==0)
        # show(instance_mask[0][1])

        img_feats_pre,prev_bev = self.obtain_history_bev(prev_img, prev_img_metas) if len_queue>1 else None
        ...
        losses_pts = self.forward_pts_train(img_feats, lidar_feat, gt_bboxes_3d,
                                            ...
                                            gt_bboxes_ignore, prev_bev,semantic_mask,instance_mask,semantic_gt,img_feats_pre,prev_gt_bboxes_3d,prev_gt_labels_3d,gt_depth,)
    ...
    def forward_test(self, img_metas, img=None,points=None,  **kwargs):
        ...
        #
        # outs = self.pts_bbox_head(
        #     img_feats, None, img_metas, prev_bev=pre, pre_feat=img_feats_pre)
        ...
        show = False
        if show:
            now_bev = rearrange(new_prev_bev, '(h w) b  c -> b c h w', h=200, w=100)
            prev_bev_vis = rearrange(now_bev, 'b c h w -> b h w c')
    ...
    def simple_test(self, img_metas, img=None, points=None, prev_bev=None, rescale=False, **kwargs):
        ...
        if self.modality =='fusion':
            lidar_feat = self.extract_lidar_feat(points)
        img_feats = self.extract_feat(img=img, img_metas=img_metas)#1 6 256 15 25
