...
import json
import cv2
from .nuscenes_map_dataset import RasterizeMapVectors
...
class VectorizedLocalMap(object):
    ...
    def __init__(self,
        ...
        for loc in self.MAPS:
            self.nusc_maps[loc] = NuScenesMap(dataroot='xx', map_name=loc)
    ...
    def get_map(self,location, lidar2global_translation, lidar2global_rotation):
            ...
            if vec_class == 'divider':
                ...
                for line_type, vects in line_vector_dict.items():
                    for line, length in vects:
                        vec.append((line.astype(float), length, self.CLASS2LABEL_vec.get(line_type, -1)))
                ...
                for ped_line, length in ped_vector_list:
                    vec.append((ped_line.astype(float), length, self.CLASS2LABEL_vec.get('ped_crossing', -1)))
                ...
                for contour, length in poly_bound_vec:
                    vec.append((contour.astype(float), length, self.CLASS2LABEL_vec.get('contours', -1)))
            else:
    ...
    def gen_vectorized_samples(self, map_annotation, example=None, feat_down_sample=32):
        ...
        gt_labels = []
        gt_instance = []
        instance_map = [torch.tensor(np.zeros((self.canvas_size[0], self.canvas_size[1]), dtype=np.uint8))]
        if self.aux_seg['use_aux_seg']:
                ...
                for instance, instance_type in vectors:
                    if instance_type != -1:
                        ...
                        if instance.geom_type == 'LineString':
                            if self.aux_seg['bev_seg']:
                                vec_map = self.vec_mask(instance, color=1, thickness=self.thickness)
                                instance_map.append(torch.tensor(vec_map))
                                gt_semantic_mask[0]=self.line_ego_to_mask(instance, gt_semantic_mask[0], color=1, thickness=self.thickness)
                            if self.aux_seg['pv_seg']:
                                for cam_index in range(num_cam):
                                    gt_pv_semantic_mask[cam_index][0]=self.line_ego_to_pvmask(instance, gt_pv_semantic_mask[cam_index][0], lidar2feat[cam_index],color=1, thickness=self.aux_seg['pv_thickness'])
                        else:
            ...
            instance_group = None#[torch.tensor(np.zeros((self.canvas_size[0], self.canvas_size[1]), dtype=np.uint8))]
            for instance, instance_type in vectors:
                if instance_type != -1:
                    gt_instance.append(instance)
                    gt_labels.append(instance_type)
    ...
    def line_ego_to_pvmask(self,
        ...
        m=cv2.polylines(mask, np.int32([pix_coords]), False, color=color, thickness=thickness)
        return m
    def line_ego_to_mask(self, 
        ...
        a = affinity.scale(line_ego, self.scale_x, self.scale_y, origin=(0, 0))
        a = affinity.affine_transform(a, [1.0, 0.0, 0.0, 1.0, trans_x, trans_y])
        # print(np.array(list(line_ego.coords), dtype=np.int32).shape)
        coords = np.array(list(a.coords), dtype=np.int32)[:, :2]
        coords = coords.reshape((-1, 2))
        assert len(coords) >= 2
        
        m=cv2.polylines(mask, np.int32([coords]), False, color=color, thickness=thickness)
        return m

    def vec_mask(self,line_ego,
                        color=1,
                        thickness=3):
        mask = np.zeros(( self.canvas_size[0], self.canvas_size[1]), dtype=np.uint8)
        trans_x = self.canvas_size[1] / 2
        trans_y = self.canvas_size[0] / 2
        a = affinity.scale(line_ego, self.scale_x, self.scale_y, origin=(0, 0))
        a = affinity.affine_transform(a, [1.0, 0.0, 0.0, 1.0, trans_x, trans_y])
        ...
        mask=cv2.polylines(mask, np.int32([coords]), False, color=color, thickness=thickness)
...
class CustomNuScenesOfflineLocalMapDataset(CustomNuScenesDataset):
    ...
    def vectormap_pipeline(self, example, input_dict):
        ...
        if anns_results['gt_semantic_mask'] is not None:
            ...
            example['instance_mask'] = DC(to_tensor(anns_results['instance_map']), cpu_only=False)
            example['semantic_mask'] = DC(to_tensor(anns_results['gt_semantic_mask']), cpu_only=False)
        ...
        # vec = self.vector_map.get_map(location, lidar2global_translation, lidar2global_rotation)
        # semantic_mask, instance, semantic_gt = self.raster(vec)
        # example['instance_mask'] = DC(to_tensor(anns_results['instance_map']), cpu_only=False)
        # example['semantic_mask'] = DC(to_tensor(anns_results['gt_semantic_mask']), cpu_only=False)
        return example
