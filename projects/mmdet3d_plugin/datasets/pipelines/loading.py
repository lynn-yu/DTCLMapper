...
class CustomLoadPointsFromFile:
    ...
    def __call__(self, results):
        ...
        """
        # lidar_path = '/data1/lsy/pcd/lidar_1.pcd'#+str(self.n)+'.pcd'#results["lidar_path"]
        # self.n=self.n+1
        # points = self._load_points(lidar_path)
        # p = points.shape[0] // 5
        # points = points[:p * 5].reshape(-1, self.load_dim)
        lidar_path = results["lidar_path"]
        points = self._load_points(lidar_path)
        points = points[:-1].reshape(-1, self.load_dim)
        # TODO: make it more general
        if self.reduce_beams and self.reduce_beams < 32:
...
class CustomPointToMultiViewDepth(object):
    ...
    def __call__(self, results):
        ...
        post_trans = torch.stack(
            [torch.tensor(single_aug_matrix[:3, 3]).to(torch.float) for single_aug_matrix in img_aug_matrix])
        # import pdb;pdb.set_trace()
...
@PIPELINES.register_module()
class LoadPointsFromFile_16(object):
    """Load Points From File.
