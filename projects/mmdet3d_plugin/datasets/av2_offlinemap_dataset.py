...
class VectorizedAV2LocalMap(object):
    ...
    def gen_vectorized_samples(self, map_annotation, example=None, feat_down_sample=32):
        ...
        for vec_class in self.vec_classes:
            ...
            for instance in instance_list:
                if instance.shape[0] < 2:
                    ...
                    continue
                l = LineString(np.array(instance))
                vectors.append((l, self.CLASS2LABEL.get(vec_class, -1)))
                line,length = self.sample_pts_from_line(l)
                vec.append((line.astype(float), length, self.CLASS2LABEL_vec.get(vec_class, -1)))
        ...
        gt_semantic_index = np.zeros((self.canvas_size[0], self.canvas_size[1]), dtype=np.uint8)
        ...
        if self.aux_seg['pv_seg']:
            ...
            # import ipdb;ipdb.set_trace()
            gt_pv_semantic_mask = np.zeros((num_cam, 1, img_shape[0] // feat_down_sample, img_shape[1] // feat_down_sample),
                                           dtype=np.uint8)
        ...
        for instance, instance_type in vectors:
            if instance_type != -1:
                ...
                gt_semantic_index=self.line_ego_to_mask(instance,gt_semantic_index, color=instance_type+1, thickness=self.thickness)
                instances = self.vec_mask(instance,color=1, thickness=self.thickness )
                instance_map.append(torch.tensor(instances))
                if instance.geom_type == 'LineString':
                    ...
                    if self.aux_seg['pv_seg']:
                        for cam_index in range(num_cam):
                            self.line_ego_to_pvmask(instance, gt_pv_semantic_mask[cam_index][0], lidar2feat[cam_index],
                                                    color=1, thickness=self.aux_seg['pv_thickness'])
        instance_map = torch.stack(instance_map)
        instance_group = instance_map
        # a = torch.tensor(gt_semantic_mask[0]) == 0
        # instance_group = a.float()
        # instance_group = instance_group.unsqueeze(0)
        # num = instance_map.shape[0]
        # if num != 1:
        #     p = 49 // num  # 3
        #     l = 49 % num
        #     for i in range(p):
        #         instance_group = torch.cat((instance_group, instance_map), dim=0)
        #     if l > 0:
        #         instance_group = torch.cat((instance_group, instance_map[0:l]), dim=0)
        # else:
        #     instance_group = instance_group.repeat(50, 1, 1)

        gt_semantic_index = torch.tensor(gt_semantic_index)
        gt = []
        gt.append(gt_semantic_index == 0)
        gt.append(gt_semantic_index == 1)
        gt.append(gt_semantic_index == 2)
        gt.append(gt_semantic_index == 3)
        gt_semantic_iou = torch.stack(gt)
        ...
        anns_results = dict(
            ...
            gt_semantic_iou = gt_semantic_iou,
            gt_semantic_index = gt_semantic_index,
            gt_instance=instance_group,
    ...
    def vec_mask(self,line_ego,
        ...
        return mask
    def seg_mask(self,mask,line_ego,
                        color=1,
                        thickness=3):
...
class CustomAV2OfflineLocalMapDataset(CustomNuScenesDataset):
    ...
    def vectormap_pipeline(self, example, input_dict):
        ...
        example['semantic_mask'] = DC(anns_results['gt_semantic_index'], cpu_only=False)
        example['instance_mask'] = DC(anns_results['gt_instance'], cpu_only=False)
        example['semantic_gt'] = DC(anns_results['gt_semantic_iou'], cpu_only=False)
        ...
        # if anns_results['instance_map'] is not None:
        #     example['gt_instance_map'] = DC(to_tensor(anns_results['instance_map']), cpu_only=False)
        return example
    ...
    def prepare_train_data(self, index):
        ...
        for i in prev_indexs_list:
            ...
            if input_dict is None:
                return None
            if input_dict['timestamp'] < frame_idx and input_dict['log_id'] == scene_token:
                self.pre_pipeline(input_dict)
                example = self.pipeline(input_dict)
                example = self.vectormap_pipeline(example, input_dict)
                if self.filter_empty_gt and \
