...
import json
from .rasterize import preprocess_map#xg
from .metrics import IntersectionOverUnion#xg
import torchvision
from tools.iou import get_batch_iou
...
class VectorizedLocalMap(object):
    ...
    def __init__(self,
        ...
        self.patch_size = patch_size#(60 30)(x y)
        self.sample_dist = sample_dist
        self.num_samples = num_samples
        self.padding = padding
    ...
    def gen_vectorized_samples(self, location, lidar2global_translation, lidar2global_rotation):
        ...
        patch_box = (map_pose[0], map_pose[1], self.patch_size[0], self.patch_size[1])#（x,y,60,30)，x对应w为30
        patch_angle = quaternion_yaw(rotation) / np.pi * 180
        vectors = []
        vec = []
        for vec_class in self.vec_classes:
            if vec_class == 'divider':
                line_geom,_ = self.get_map_geom(patch_box, patch_angle, self.line_classes, location)
                line_instances_dict,line_vector_dict = self.line_geoms_to_instances(line_geom)
                for line_type, vects in line_vector_dict.items():
                    for line, length in vects:
                        vec.append((line.astype(float), length, self.CLASS2LABEL.get(line_type, -1)))
                for line_type, instances in line_instances_dict.items():
                    for instance in instances:
                        vectors.append((instance, self.CLASS2LABEL.get(line_type, -1)))
            elif vec_class == 'ped_crossing':
                ped_geom,ped_line_geom = self.get_map_geom(patch_box, patch_angle, self.ped_crossing_classes, location)
                ped_instance_list,_ = self.ped_poly_geoms_to_instances(ped_geom)
                for instance in ped_instance_list:
                    vectors.append((instance, self.CLASS2LABEL.get('ped_crossing', -1)))
                ped_vector_list = self.line_geoms_to_vectors(ped_line_geom)['ped_crossing']
                for ped_line, length in ped_vector_list:
                    vec.append((ped_line.astype(float), length, self.CLASS2LABEL.get('ped_crossing', -1)))
            elif vec_class == 'boundary':
                polygon_geom,_ = self.get_map_geom(patch_box, patch_angle, self.polygon_classes, location)
                poly_bound_list,poly_bound_vec = self.poly_geoms_to_instances(polygon_geom)
                for contour in poly_bound_list:
                    vectors.append((contour, self.CLASS2LABEL.get('contours', -1)))
                for contour, length in poly_bound_vec:
                    vec.append((contour.astype(float), length, self.CLASS2LABEL.get('contours', -1)))
            else:
    ...
    def get_map_geom(self, patch_box, patch_angle, layer_names, location):
        ...
        for layer_name in layer_names:
            if layer_name in self.line_classes:
                ...
                geoms,line = self.get_ped_crossing_line(patch_box, patch_angle, location)
                map_geom.append((layer_name, geoms))
                ped_geom.append((layer_name, line))
        return map_geom,ped_geom
    ...
    def ped_poly_geoms_to_instances(self, ped_geom):
        ...
        return self._one_type_line_geom_to_instances(results),self._one_type_line_geom_to_vectors(results)
    ...
    def line_geoms_to_instances(self, line_geom):
        ...
        return line_instances_dict,line_vectors_dict
...
class RasterizeMapVectors(object):
    ...
    def __init__(self,
        ...
        patch_w = map_ybound[1] - map_ybound[0]#30
        patch_h = map_xbound[1] - map_xbound[0]#60

        # canvas_size: 在 y, x 方向上的 bev 尺寸
        canvas_h = int(patch_h / map_xbound[2])#400
        canvas_w = int(patch_w / map_ybound[2])#200

        self.map_patch_size = (patch_h, patch_w)#60 30
        self.map_canvas_size = (canvas_h, canvas_w)#200 100

    def __call__(self,vectors):

        for vector in vectors:
            vector['pts'] = vector['pts'][:, :2]
        # all_instance=instance.fixed_num_sampled_points
        # vectors=[]
        # for i in range(len(lable)):
        #     vectors.append({'pts':all_instance[i],'type':lable[i],'pts_num':20})

        instance_masks,semantic_indices = preprocess_map(
            vectors, self.map_patch_size, self.map_canvas_size, self.map_max_channel, self.map_thickness, self.map_angle_class)#N 200 100
        #instance=(semantic_indices == 0)#200 100
        # instance =instance.unsqueeze(0)
        # num = instance_masks.shape[0]-1
        # if num!=0:
        #     p=49//num#3
        #     l= 49%num
        #     for i in range(p):
        #         instance=torch.cat((instance,instance_masks[1:]),dim=0)
        #     if l>0:
        #         instance=torch.cat((instance,instance_masks[1:l+1]),dim=0)
        # else:
        #     instance = instance.repeat(50,1,1)
        # plt.figure()
        # for i in range(1,50):
        #     plt.imshow(instance[i])
        # plt.show()
        #assert instance.shape[0]==50
        gt =[]
        gt.append(semantic_indices==0)
        gt.append(semantic_indices == 1)
        gt.append(semantic_indices == 2)
        gt.append(semantic_indices == 3)
        semantic_gt = torch.stack(gt)

        # semantic_masks = instance_masks != 0#
        # instance_masks = instance_masks.sum(0)
        # semantic_gt =  semantic_masks
        # semantic_gt = torch.cat([(~torch.any(semantic_gt, axis=0)).unsqueeze(0), semantic_gt])
        # num_cls = semantic_masks.shape[0]#3 200 100
        # indices = torch.arange(1, num_cls + 1).reshape(-1, 1, 1)
        # semantic_indices = torch.sum(semantic_masks * indices, dim=0)

        # one = torch.ones_like(semantic_indices)
        # semantic_indices += one
        return semantic_indices,instance_masks,semantic_gt
...
class CustomNuScenesLocalMapDataset(CustomNuScenesDataset):
    ...
    def __init__(self,
        ...
        patch_h = pc_range[4] - pc_range[1]#x
        patch_w = pc_range[3] - pc_range[0]#y
        self.patch_size = (patch_h, patch_w)#60 30
        self.padding_value = padding_value
        self.fixed_num = fixed_ptsnum_per_line
    ...
    def init_layer(self):
        ...
        self.raster = RasterizeMapVectors(map_grid_conf)
    ...
    def vectormap_pipeline(self, example, input_dict):
        ...
        semantic_mask,instance,semantic_gt = self.raster(anns_results['vectors'])
        example['semantic_mask'] = DC(semantic_mask, cpu_only=True)
        example['instance_mask'] = DC(instance, cpu_only=True)
        example['semantic_gt'] = DC(semantic_gt, cpu_only=True)
        return example
    ...
    def union2one(self, queue):
        ...
        for i, each in enumerate(queue):
            ...
            if i == 0:
                ...
                pre_motion = metas_map[i]['ego_motion']
                ...
                cur_motion = metas_map[i]['ego_motion']
                ego_motion = invert_matrix_egopose_numpy(
                    cur_motion).dot(pre_motion)
                ego_motion[3, :3] = 0.0
                ego_motion[3, 3] = 1.0
                metas_map[i]['pre2now_futuremotion'] = ego_motion

        queue[-1]['img'] = DC(torch.stack(imgs_list),
                              cpu_only=False, stack=True)
        queue[-1]['img_metas'] = DC(metas_map, cpu_only=True)
        queue[-1]['prev_gt_labels_3d'] =queue[0]['gt_labels_3d']
        queue[-1]['prev_gt_bboxes_3d'] = queue[0]['gt_bboxes_3d']
        queue = queue[-1]
        return queue
    ...
    def get_data_info(self, index):
        ...
        input_dict = dict(
            ...
            lidar_path=info["lidar_path"],#'/data1/lsy/1.pcd',#
            sweeps=info['sweeps'],
    ...
    def _evaluate_single(self,
        ...
        for metric in metrics:
            ...
            if metric == 'chamfer':
                thresholds = [0.5, 1.0, 1.5]#[0.2,0.5,1.0]#
            elif metric == 'iou':
